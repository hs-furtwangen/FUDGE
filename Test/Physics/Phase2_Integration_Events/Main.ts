...
namespace FudgePhysics_Communication {
  ...
  function init(_event: Event): void {
    ...
    let trigger: f.Node = createCompleteMeshNode("Trigger", new f.Material("Cube", f.ShaderFlat, new f.CoatRemissive(new f.Color(0, 1, 0, 1))), new f.MeshSphere(), 1, f.BODY_TYPE.STATIC, f.COLLISION_GROUP.DEFAULT, f.COLLIDER_TYPE.SPHERE);
    trigger.getComponent(f.ComponentRigidbody).isTrigger = true;
    let cmpCubeTransform4: f.ComponentTransform = trigger.getComponent(f.ComponentTransform);
    ...
    trigger.getComponent(f.ComponentRigidbody).addEventListener(f.EVENT_PHYSICS.TRIGGER_ENTER, onTriggerEnter);
    ground.getComponent(f.ComponentRigidbody).addEventListener(f.EVENT_PHYSICS.COLLISION_EXIT, onCollisionExit);
    trigger.getComponent(f.ComponentRigidbody).addEventListener(f.EVENT_PHYSICS.TRIGGER_EXIT, onTriggerExit);
