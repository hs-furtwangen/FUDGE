namespace RaySceneVR {
  ...
  export class RayHelper extends f.ComponentScript {
    ...
    public constructor(_xrViewport: f.XRViewport, _controller: f.VRController, _lengthRay: number, _cubeContainer: f.Node) {
      super();
    ...
    private computeRay = (): void => {
      if (!this.hasObject) {
        ...
        let forward: f.Vector3 = this.node.mtxWorld.getForward();
        // forward = f.Vector3.Z();
