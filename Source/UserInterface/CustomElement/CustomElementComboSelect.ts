  ...
  export class CustomElementComboSelect extends CustomElement {
    ...
    private hndChange = async (_event: Event): Promise<void> => {
      ...
      switch (this.getAttribute("action")) {
        case "create":
          this.dispatchEvent(new CustomEvent(EVENT.CREATE, { bubbles: true, detail: { type: this.value } }));
          break;
        case "assign":
          this.dispatchEvent(new CustomEvent(EVENT.ASSIGN, { bubbles: true, detail: { value: this.value } }));
          break;
      }
    };
