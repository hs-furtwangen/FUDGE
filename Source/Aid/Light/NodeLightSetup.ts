namespace FudgeAid {
  ...
  export function addStandardLightComponents(
    ...
    let key: ƒ.ComponentLight = new ƒ.ComponentLight(ƒ.LIGHT_TYPE.DIRECTIONAL, _clrKey);
    key.mtxPivot.translate(_posKey);
    key.mtxPivot.lookAt(ƒ.Vector3.ZERO());

    let back: ƒ.ComponentLight = new ƒ.ComponentLight(ƒ.LIGHT_TYPE.DIRECTIONAL, _clrBack);
    back.mtxPivot.translate(_posBack);
    back.mtxPivot.lookAt(ƒ.Vector3.ZERO());

    let ambient: ƒ.ComponentLight = new ƒ.ComponentLight(ƒ.LIGHT_TYPE.AMBIENT, _clrAmbient);
