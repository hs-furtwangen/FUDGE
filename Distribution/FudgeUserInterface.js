    ...
    class Controller {
        ...
        static updateUserInterface(_mutable, _domElement, _mutator, _parentMutable, _parentKey) {
            ...
            for (const key in mutator) {
                ...
                const btnRevert = element.querySelector(`button[name="revert"]`);
                if (btnRevert)
...
(function (FudgeUserInterface) {
    ...
    class Generator {
         ...
         */
        static createInterfaceFromFlatMutable(_mutable) {
            const mutator = ƒ.Mutable.getMutator(_mutable);
            const types = ƒ.Mutable.getMutatorTypes(_mutable, mutator);
            const descriptors = ƒ.Metadata.getPropertyDescriptors(_mutable);
            const div = document.createElement("div");
            for (const key in mutator) {
        ...
        static createInterfaceElement(_mutable, _mutator, _key, _type, _descriptor, _parentMutable, _parentKey) {
            ...
            if (element && _descriptor) {
                ...
                const revertable = Reflect.has(_descriptor, "defaultValue");
                const defaultValue = _descriptor.defaultValue;
                const menu = Generator.createInterfaceElementMenu(typeName, !!_descriptor.getCreateOptions, !!_descriptor.getAssignOptions, creatable, clearable, deletable, revertable, defaultValue);
        ...
        static createInterfaceElementMenu(_type, _createOptions, _assignOptions, _creatable, _clearable, _deletable, _revertable, _defaultValue) {
            ...
            if (_revertable) {
                ...
                btnRevert.addEventListener("click" /* EVENT.CLICK */, _event => {
                    btnRevert.dispatchEvent(new CustomEvent("assign" /* EVENT.ASSIGN */, { bubbles: true, detail: { value: ƒ.clone(_defaultValue) } }));
                    menu.close();
                });
            }
            if (_deletable) {
